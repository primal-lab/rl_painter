...
def train(config):
    ...
    LOG_EVERY_STEPS = int(config.get("log_every_steps", 200))  # throttle weight logs
    VIDEO_EVERY      = int(config.get("video_every", 50))      # throttle GIFs
    ...
    # BEGIN WEIGHTS VIZ
    # Capture grad means DURING backward inside DDPGAgent (cheap).
    # Your DDPGAgent should call self.viz_after_backward() immediately after .backward()
    # and before optimizer.step() for both critic and actor updates.
    def _viz_after_backward():
        # These reads are on GPU and very fast; no logging here.
    ...
    # Baselines for update ratio (GPU tensors)
    actor_prev_vec  = flat_params(actor,  device)
    critic_prev_vec = flat_params(critic, device)

    # Occasional CPU snapshot (rare) to ensure weights actually moved across episodes
    ...
    for episode in range(start_episode, start_episode + config["episodes"]):
        ...
        # w and b
        # Only collect frames for some episodes (cheap otherwise)
        ...
        while not done:
            ...
            # w and b
            # ----- Per-step GIF frames only for selected episodes -----
            if is_video_episode:
            ...
            # Save 1st episode's and every 50th episode's final step (2000th step)
            if ((episode + 1) == 1 or (episode + 1) % 50 == 0) and env.used_strokes == config["max_strokes"] - 1:
        ...
        # w and b
        if is_video_episode and episode_frames:
        ...
        # BEGIN WEIGHTS VIZ
        # ====== Rare CPU sanity delta (every 10 eps) ======
        if (episode + 1) % 10 == 0:
